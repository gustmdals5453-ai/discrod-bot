...
client.on("messageCreate", async message => {
  ...
  // 🏆 돈 랭킹
  if(cmd==="랭킹"){
    const ranking = Object.entries(data).sort((a,b)=>b[1].money-a[1].money).slice(0,10);
    const embed = new EmbedBuilder().setColor(0xFFD700).setTitle("🏆 돈 랭킹 TOP 10");
    let desc = "";
    for(let i=0;i<ranking.length;i++){
      const uid = ranking[i][0], money = ranking[i][1].money;
      desc += `${i+1}. <@${uid}> - 💰 ${format(money)}원\n`;
    }
    embed.setDescription(desc || "랭킹 데이터가 없습니다.");
  ...
  if(cmd==="문의"){
    ...
    const embed = new EmbedBuilder()
      .setColor(0x00B0FF)
      .setTitle("📩 고객센터")
      .setDescription("📌 1인 1문의 제한\n📌 관리자만 종료 가능\n버튼을 눌러 문의 생성")
      .setFooter({ text:"빠르게 도와드립니다 🚀" });

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId("ticket_create").setLabel("문의하기").setEmoji("📩").setStyle(ButtonStyle.Success)
    );
  ...
  if(cmd==="경고"){
    ...
    warns[target.id].push({ by: message.author.id, reason, date: Date.now() });
    const embed = new EmbedBuilder()
      .setColor(0xFF5252)
      .setTitle("⚠️ 경고 부여")
      .setDescription(`<@${target.id}> 님에게 경고가 부여되었습니다\n사유: ${reason}\n총 경고: ${warns[target.id].length}회`);
    return message.reply({ embeds:[embed] });
  }
  ...
  if(cmd==="경고해제"){
    ...
    const embed = new EmbedBuilder()
      .setColor(0x00E676)
      .setTitle("✅ 경고 해제")
      .setDescription(`<@${target.id}> 님의 마지막 경고가 해제되었습니다\n남은 경고: ${warns[target.id].length}회`);
    return message.reply({ embeds:[embed] });
  }
  ...
  if(cmd==="경고확인"){
    ...
    const embed = new EmbedBuilder()
      .setColor(0x00B0FF)
      .setTitle(`⚠️ ${target.id===id?"내":"<@"+target.id+">"} 경고 내역`)
      .setDescription(list.length===0 ? "경고 없음" : list.map((w,i)=>`${i+1}. ${w.reason} (by <@${w.by}>)`).join("\n"));
    return message.reply({ embeds:[embed] });
  }
});
...
client.on("interactionCreate", async i=>{
  ...
  if(i.customId==="ticket_create"){
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId("ticket_close").setLabel("문의 종료").setEmoji("🔒").setStyle(ButtonStyle.Danger)
    );
