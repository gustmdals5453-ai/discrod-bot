...
client.on("messageCreate", async (message) => {
  ...
  if (cmd === "잔액") {
    return message.reply({
      embeds:[new EmbedBuilder()
        .setColor(0x00E5FF)
        .setTitle("💳 내 잔액")
        .setDescription(`💰 ${format(data[id].money)}원`)]
    });
  }
  ...
  if (cmd === "슬롯") {
    ...
    let msg = await message.reply({
      embeds:[new EmbedBuilder().setColor(0x6C5CE7).setTitle("🎰 슬롯").setDescription("🎲 준비 중...")]
    });

    let s1,s2,s3;

    for(let i=0;i<5;i++){
      s1=getRandomSymbol(); s2=getRandomSymbol(); s3=getRandomSymbol();
      await msg.edit({
        embeds:[new EmbedBuilder()
          .setColor(0x6C5CE7)
          .setTitle("🎰 슬롯")
    ...
    const f1=getRandomSymbol(), f2=getRandomSymbol(), f3=getRandomSymbol();
    ...
    await msg.edit({
      embeds:[new EmbedBuilder()
        .setColor(win>0?0x00E676:0xFF1744)
        .setTitle("🎰 슬롯 결과")
  ...
  if (cmd === "가위바위보") {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId("rps_가위").setEmoji("✌️").setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("rps_바위").setEmoji("✊").setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("rps_보").setEmoji("✋").setStyle(ButtonStyle.Secondary)
    );
    ...
    return message.reply({
      embeds:[new EmbedBuilder()
        .setColor(0x7C4DFF)
        .setTitle("🎮 가위바위보")
        .setDescription(`💸 ${format(bet)}원 배팅\n\n👇 버튼 선택`)],
      components:[row]
    });
  }
  ...
  if (cmd === "랭킹") {
    ...
    const desc = sorted.map((u,i)=>`🏆 ${i+1}위 <@${u[0]}> - ${format(u[1].money)}원`).join("\n");

    return message.reply({
      embeds:[new EmbedBuilder().setColor(0xFFD600).setTitle("📊 서버 랭킹").setDescription(desc||"없음")]
    });
  }
  ...
  if (cmd === "조작") {
    if (message.author.id !== ownerId)
      return message.reply("❌ 관리자 전용");

    const target = message.mentions.users.first();
    const amount = parseInt(args[1]);

    if (!target || isNaN(amount))
      return message.reply("❌ !조작 @유저 금액");
    ...
    return message.reply({
      embeds:[new EmbedBuilder()
        .setColor(0xFFEA00)
        .setTitle("🛠️ 금액 조작 완료")
        .setDescription(`<@${target.id}> → ${format(amount)}원`)]
    });
  }
    ...
    if (tickets[id]) {
      return message.reply({
        embeds:[new EmbedBuilder()
          .setColor(0xFF5252)
          .setTitle("❌ 이미 문의 있음")
          .setDescription("이미 문의 채널이 존재합니다")]
      });
    }
...
client.on("interactionCreate", async (i) => {
  ...
  if (i.customId.startsWith("rps_")) {
    ...
    if ((user==="가위"&&bot==="보")||(user==="바위"&&bot==="가위")||(user==="보"&&bot==="바위")){
      change=bet; result="승리 🎉";
    } else if(user!==bot){
      change=-bet; result="패배 💀";
    }
    ...
    return i.update({
      embeds:[new EmbedBuilder()
        .setColor(change>0?0x00E676:0xFF1744)
        .setTitle("🎮 결과")
        .setDescription(`${emojis[user]} vs ${emojis[bot]}\n\n${result}\n\n💰 ${change>0?"+":""}${format(change)}원\n💳 ${format(data[id].money)}원`)],
      components:[]
    });
  }
